...
lin.quad.peak <- function(col.numbers,dataset,error.distribution){

   for(i in min(col.numbers):max(col.numbers)){
      diff.BIC <- BIC(glm(Peak~poly(dataset[,i],1),data=dataset,
                              family=error.distribution))-
                     BIC(glm(Peak~poly(dataset[,i],2),data=dataset,
                              family=error.distribution))
